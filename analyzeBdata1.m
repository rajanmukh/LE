function [h,detstat]=analyzeBdata1(msg,ID,pos,refFreq,BRT,bID,foa,foff,toa,CNR,antNo,SIDa,pXYZ,vXYZ)
% close all
load('prns.mat','prns')
load('clrs.mat','clrs')
...
foaC=foaB(chnmatch);
toaC=toaB(chnmatch);
% expT=cell(1,length(msgC));
% expF=cell(1,length(msgC));
TxSite=lla2ecef([43.56,1.48,214])'*1e-3;
for i=1:length(msgC)
    data = hexToBinaryVector(msgC{i},144);
    ...
    [expT,expF]=TRxOperation(SIDc(i),tot,406.022e6,TxSite,LUTxyz);
    trr(i)=seconds(toaC(i)-expT);
    frr(i)=foaC(i)-expF;
end
