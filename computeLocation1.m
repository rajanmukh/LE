function [lat, lon, height,EHE,elp,resd,extraS] = tdoa3_rpt(datetoa,foa,cnr,sats)
global LIGHTSPEED;
...
if ~largeresd
    ...
    if any(erroneous_channel)
        lat=NaN;    lon=NaN;    height=NaN;    EHE=NaN;    elp=[];extraS=[];
    else
        [jdop,elp]=computeDOP(D,lat*pi/180,lon*pi/180);
        EHE = 2.5*jdop;
        sigt=median(stdtoa);
        extraS.jdop=jdop/sigt;
        extraS.EHE=EHE;
        extraS.ft=mean(ft);
        extraS.ants=chn;
    end
...
for i=1:7
    if ~isempty(datetoa{i})
        ...
        dt=datestr{1};
        DDD=dt(6:end);
        date=datetime(datestr{1},'InputFormat','uuuu-DDD');
        d=str2double(DDD);
        tme=split(datestr{2},':');
        toa(j)=[3600 60 1 1e-9]*str2double(tme)+0.16;
