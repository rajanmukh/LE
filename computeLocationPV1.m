function [loc,err,antsV,sInfo] = computeLocationPV1(toas,foas,cnrs,satIDs)
loc=[];
err=[];
sInfo=[];
%constant
...
%     G(5)=mean(fc1);
    ...
    if resd>100
        ...
        [G,antsV,errorEliminated] = tryElimination(satIDs,posS,t,posS1,velS1,fc1,stdtoa,stdfoa,tryOrder);        
    else
        errorDetected=false;
    ...
    if ~errorDetected || (errorDetected && errorEliminated)
        ...
        velS3=velS1(:,antsV);
        fc3=fc1(antsV);
        stdfoa1=stdfoa(antsV);
        for i=1:15
            [F1,D1]=FDcreator3(posS3,velS3,fc3,G,location_est);
            F2=F1./stdfoa1;
            D2=D1./stdfoa1;
            del=D2\F2;
            G=G-del;
        ...
        ft=fc1-fd1;
        llaPos=ecef2lla(1e3*location_est');
        loc.lat=llaPos(1);
        loc.lon=llaPos(2);
        loc.alt=llaPos(3)/1e3;
        [course,Vh,Vv]=computeVel(G(1:3),loc.lat*pi/180,loc.lon*pi/180)
        vEHE=1e3*2.5*computeDOP(D2,loc.lat*pi/180,loc.lon*pi/180);
...
function [F,D] = FDcreator3(posS,velS,freq,G,xyz)
global LIGHTSPEED;
noOfSat=length(freq);
F=zeros(noOfSat,1);
vxyz=G(1:3);
