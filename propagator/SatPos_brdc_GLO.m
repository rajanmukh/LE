function [X, V] = SatPos_brdc_GLO(Ttr, prn)
...
% get variables
x     = Eph(5,index);            % PZ90, [km]
y     = Eph(6,index);
z     = Eph(7,index);
v_x = Eph(8,index);              % PZ90, [km/s]
v_y = Eph(9,index);
v_z = Eph(10,index);
a_x = Eph(11,index);         	% PZ90, [km/s^2]
a_y = Eph(12,index);
a_z = Eph(13,index);
toe = Eph(18,index);             % epoch of ephemerides, GPS time [sow]
